        ...
        if(!GetIsReactionTypeFriendly(oTarget))
        {
            ...
            if (!MyResistSpell(OBJECT_SELF, oTarget, fDelay) && (oTarget != OBJECT_SELF))
            {
                ...
                if(nRandom == 8)
                {
                 ...
                 //   nVisual = ApplyPrismaticEffect(Random(7) + 1, oTarget);
                 //so death wont work
                   nVisual = ApplyPrismaticEffect(Random(6) + 1, oTarget);
                   nVisual = ApplyPrismaticEffect(Random(6) + 1, oTarget);
                }
                else
                {
                    //Get the visual effect
                    //reroll to match the new range of 1-6
                    nVisual = ApplyPrismaticEffect(Random(6) + 1, oTarget);
                 //original   nVisual = ApplyPrismaticEffect(nRandom, oTarget);
                }
        ...
        case 5: //Paralyze
            {
                ...
                if (MySavingThrow(SAVING_THROW_FORT, oTarget, GetSpellSaveDC()) == 0)
                {
                    ...
                    int Duration = 3;
                    DelayCommand(fDelay, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eLink, oTarget, RoundsToSeconds(Duration)));
                    SetLocalInt(oTarget,"hp_before_stun",GetCurrentHitPoints(oTarget));
                    InitiateStunCheck (oTarget, Duration);
                }
            }
        break;
        case 6: //Confusion
