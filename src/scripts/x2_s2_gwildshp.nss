...
//:://////////////////////////////////////////////
//change: 2.91 - add monk AC malus
//change: 7.04 - own ItemPropertyCopy scripts
//change: 9.06 - re-check for shifter if he is carrying flag/inside domarea
...
// ----------------------------------------------------------------------------
void NWK_IPWildShapeCopyItemProperties(object oOld, object oNew, int bWeapon = FALSE)
{
    if (GetIsObjectValid(oOld) && GetIsObjectValid(oNew))
    {
        ...
        {
            // Execute copy only if duration type is permanent.
            // This will prevent change of duration type from temporary
            // to permanent. Else time limited weapon buffs get same
            // duration as the duration of the shifters form.
...
void main()
{
    ...
    if (bWeapon)
    {
        ...
        else*/
        {
            ...
            //------------------------------------------------------------------
            NWK_IPWildShapeCopyItemProperties(oWeaponOld,oWeaponNew, TRUE);
        }
    }
    ...
    AddItemProperty ( DURATION_TYPE_PERMANENT,
                      ...
                      oArmorNew );
    addACDecreaseIfPCIsMonk(OBJECT_SELF);

    // check for flag and dominator inside dominated area
    if ( GetHasFlag ( OBJECT_SELF ) || GetIsActiveDominator ( OBJECT_SELF ) )
    {
        // re-assign hide nerfs
        nerfPc ( OBJECT_SELF );
    }
