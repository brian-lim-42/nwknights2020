...
void removeInvisibilitySpell(object oPC);
void removeInvisibilitySpell(object oPC)
{
   RemoveSpecificEffect(EFFECT_TYPE_INVISIBILITY, oPC);
   RemoveSpecificEffect(EFFECT_TYPE_SANCTUARY, oPC);
   RemoveSpecificEffect(EFFECT_TYPE_ETHEREAL, oPC);
}

//used in replacement of slow spells - only reduce speed instead of also num-of-attacks
effect createSlowSpeedEffect();
effect createSlowSpeedEffect()  {
