    ...
    if (spellsIsTarget(oTarget, SPELL_TARGET_STANDARDHOSTILE, GetAreaOfEffectCreator()))
    {
        ...
        if(!MyResistSpell(GetAreaOfEffectCreator(), oTarget, fDelay))
        {
           ...
           if (GetHasSpellEffect(453, oTarget) == FALSE)
           {
                ...
                {
                    ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eSpeed, oTarget, RoundsToSeconds(1));
                }
           }
        }
    }
    ...
    if(!GetIsReactionTypeFriendly(oTarget))
    {
        ...
        switch (nGameDiff)
        {
           ...
           //         fDifficulty = RoundsToSeconds(nPower); // One Round per hit-die or caster level
                   fDifficulty = RoundsToSeconds(3); // YAN = One Round per hit-die or caster level
                    break;
        //    case GAME_DIFFICULTY_CORE_RULES:
        //    case GAME_DIFFICULTY_DIFFICULT:
        //        bShowPopup = TRUE;
        ...
        fDifficulty = RoundsToSeconds(3) ; //PENTAGON - TO MAKE SURE , EVEN IF DIFFICULTY IS OTHER
