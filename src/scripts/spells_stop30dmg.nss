/* spells_stopstun
 * this is a script that should be used by all the stun damaging spells.
 * currently bigbys.
 * it checks if the target recieved any dmg above 30 during last round and if so,
...
void main()
{
 ...
 // SendMessageToPC(oPC,"insude the remove effect loop");
     ...
     {  //if it is bigger(healed) or less(minor dmg) just save the new one as hp_beofre_stun
